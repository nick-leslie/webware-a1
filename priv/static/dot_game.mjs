...
// build/dev/javascript/dot_game/grid.mjs
...
var Clear = class extends CustomType {
};
...
function new_grid(wh) {
  let tiles = (() => {
    let _pipe = range(0, wh[0]);
    let _pipe$1 = map(
      _pipe,
      (y) => {
        let _pipe$12 = range(0, wh[0]);
        return map(
          _pipe$12,
          (x) => {
            let cords = [x, y];
            return new Tile(cords, new Clear());
          }
        );
      }
    );
    ...
    return fold(
      ...
      new_map(),
      (acc, node) => {
        return insert(acc, node.cords, node);
      }
    );
  })();
  return new Grid(tiles, wh);
}
function get_tile(grid, cords) {
  return map_get(grid.tile, cords);
}
function set_tile(grid, tile) {
  return grid.withFields({ tile: insert(grid.tile, tile.cords, tile) });
}

// build/dev/javascript/dot_game/dot_game.mjs
...
var ChangeColor = class extends CustomType {
  constructor(tile) {
    super();
    this.tile = tile;
  }
};
var ChangePallet = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
...
function init2(size) {
  return [new Model2(new_grid(size), new Blue()), none()];
}
function update(model, msg) {
  if (msg instanceof ChangeColor) {
    let tile = msg.tile;
    let new_tile = tile.withFields({ color: model.color });
    return [
      model.withFields({ grid: set_tile(model.grid, new_tile) }),
      none()
    ];
  } else {
    let color = msg[0];
    return [model.withFields({ color }), none()];
  }
}
function pallet_view() {
  return div(
    toList([class$("col")]),
    toList([
      div(
        toList([
          style(toList([["background-color", "blue"]])),
          on_click(new ChangePallet(new Blue()))
        ]),
        toList([])
      ),
      div(
        toList([
          style(toList([["background-color", "white"]])),
          on_click(new ChangePallet(new Clear()))
        ]),
        toList([])
      ),
      div(
        toList([
          style(toList([["background-color", "green"]])),
          on_click(new ChangePallet(new Green()))
        ]),
        toList([])
      ),
      div(
        toList([
          ...
          on_click(new ChangePallet(new Red()))
        ]),
        toList([])
      ),
      div(
        toList([
          style(toList([["background-color", "yellow"]])),
          on_click(new ChangePallet(new Yellow()))
        ]),
        toList([])
      )
    ])
...
function color_to_string(color) {
  if (color instanceof Blue) {
    return "blue";
  } else if (color instanceof Clear) {
    return "white";
...
function tile_view(tile) {
  let x_string = to_string(tile.cords[0]);
  let y_string = to_string(tile.cords[1]);
  let cords_string = (() => {
    let _pipe = new$();
    let _pipe$1 = append(_pipe, "(");
    let _pipe$2 = append(_pipe$1, x_string);
    let _pipe$3 = append(_pipe$2, ",");
    let _pipe$4 = append(_pipe$3, y_string);
    let _pipe$5 = append(_pipe$4, ")");
    return identity(_pipe$5);
  })();
  return div(
    toList([
      style(
        toList([["background-color", color_to_string(tile.color)]])
      ),
      on_click(new ChangeColor(tile))
    ]),
    toList([text2(cords_string)])
  );
}
  ...
  return div(
    ...
    (() => {
      ...
      return map(
        ...
        (y) => {
          return div(
            toList([class$("flex-row")]),
            (() => {
              let _pipe$1 = range(0, grid.size[0]);
              return map(
                _pipe$1,
                (x) => {
                  let cords = [x, y];
                  debug(grid);
                  let $ = get_tile(grid, cords);
                  if ($.isOk()) {
                    let color = $[0];
                    return tile_view(color);
                  } else {
                    debug(cords);
                    throw makeError(
                      "panic",
                      ...
                      97,
                      "",
                      "we should never be out of scope in this view function",
                      {}
                    );
                  }
                }
...
function view(model) {
  return div(toList([]), toList([pallet_view(), grid_view(model.grid)]));
}
function main() {
  let app = application(init2, update, view);
  let $ = start2(app, "#app", [10, 10]);
  if (!$.isOk()) {
    throw makeError(
      "let_assert",
      "dot_game",
