...
// build/dev/javascript/todo_app/todo_app.mjs
var Model2 = class extends CustomType {
  constructor(todos, last_todo, current_input) {
    super();
    this.todos = todos;
    this.last_todo = last_todo;
...
function init2(size) {
  return [new Model2(new_map(), 0, ""), none()];
}
function update(model, msg) {
  if (msg instanceof AddTodo) {
    ...
    return [
      model.withFields({
        todos: delete$(model.todos, index2),
        last_todo: model.last_todo
      }),
      none()
    ];
  } else {
    let input2 = msg[0];
    return [model.withFields({ current_input: input2 }), none()];
  }
}
function todo_view(todo_value) {
  return div(
  ...
  return div(
    ...
    toList([
      ...
      div(
        toList([]),
        (() => {
          let _pipe = map_to_list(model.todos);
          return map(_pipe, (_capture) => {
            return todo_view(_capture);
          });
        })()
      )
    ])
